...
| TD-PROV-003 | — | Closed 2026-08-11 | Provider failure signalling | Failure was signalled by prose the router pattern-matched; three adapters used prose it did not recognise and never failed over. |
...
### TD-PROV-003 — Provider failure was signalled by prose, and three adapters used the wrong prose

- **Priority/status:** Closed 2026-08-11. Found by the 2026-08-10 silent-failure sweep.
- **Evidence:** 19 adapters answered a failed generation by *returning* `'My brain disconnected, try again.'`, and `FallbackRouter.isFailedResponse()` decided whether to try the next provider by pattern-matching that sentence. A plain English string was therefore load-bearing control flow, which produced three distinct defects:
  1. **Silent failover holes.** Three adapters returned different prose on their failure paths, which the router did not recognise, so those providers never failed over and the error text was handed to the player as the bot's answer: `claude.js` `'No response from Claude.'` (response with no text block), `gemini.js` `'An unexpected error occurred, please try again.'` — **inside the catch, so Gemini had no working failover for any error** — and `ollama.js` `'No response data.'` (empty API response).
  2. **False positives.** The router used a loose `/brain disconnected/i` substring test. A legitimate reply that merely used the phrase would have been discarded as a dead provider and the question re-asked of another one.
  3. **Lost cause.** The real error was only `console.log(err)`'d with no provider attribution, so on a routed profile you could not tell which provider failed or why.
- **Resolution:** New `src/models/provider-failure.js` owns one canonical `PROVIDER_FAILURE_TEXT` (unchanged wording, so what the player sees is identical), a `recordProviderFailure(provider, error)` helper that preserves the cause in a bounded log and logs with provider attribution, and an exact-match `isProviderFailureText()` the router now uses. All 19 adapters call the helper; the three divergent failure paths were converted so they fail over correctly. `'Vision is only supported by certain models.'` was deliberately left alone — that is a capability answer, not a transport failure, and must not trigger failover.
- **Guards:** `tests/control-plane/provider-failure.test.js` asserts no adapter hardcodes the failure text, and that any adapter calling the helper actually imports it. The second guard exists because the first sweep introduced exactly that bug in `glhf.js` and `hyperbolic.js` — a `ReferenceError` reachable only on the failure path, which `node --check` cannot see. Two further sites (`finalRes`, `completionContent`) were caught by the guard after the bulk rewrite missed their variable names.
- **Exit criteria:** One definition of the failure signal, every adapter using it, no prose false positives, and the cause recoverable after the fact. — met.
- **Deliberately not done:** adapters still *return* failure rather than throwing. Converting them to typed throws would change what the player hears when a provider dies (today they get an apology; a throw would leave the bot silent on conversation paths). With the signal now canonical and in one place, that becomes a one-file change if it is ever wanted.
