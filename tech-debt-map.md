...
| TD-TEXT-001 | P2 | Confirmed localized defect | Runtime data normalization | `rule-engine.js` destroys hyphens and preserves several control bytes; related sanitizers also encode materially different domain contracts. |
...
- **Remaining before close:** The live black-holed-endpoint probe in the exit criteria has not been run.
- **Exit criteria:** A black-holed local endpoint rejects within the configured bound, normal requests remain compatible, and fallback/backoff receives the timeout as a structured failure.
...
- **Remaining before close:** Not verified live. The exit criteria's real-generation stop still needs a running Paper world and a real provider request.
...
- **Exit criteria:** Focused examples cover hyphens, whitespace, NUL/C0 controls, DEL, fallback, and truncation; search tooling reads `rules.js` as text; no persisted format or rejection policy changes unintentionally.
