...
- [ ] Next broad tranche: start from the accepted outpost, explore and light a nearby cave, collect useful exposed ore without damaging the home site, return, and store the verified result in the bound chest. Repair only shared exploration, returnability, stewardship, or capability seams exposed by that unchanged outcome.
