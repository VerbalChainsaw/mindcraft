...
- [ ] Prove the same engine on a materially different custom functional structure with stateful or oriented components. Let that broad request expose the next missing placement or dependency contract; do not prebuild object-specific construction routes.
