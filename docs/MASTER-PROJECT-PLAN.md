...
The immediate working session begins with IronSuiteProof Operator Stop-held beside the landing-area witness after the broad starter-supply plan passed. Resume broad two-player supervised play rather than another inventory-floor or recipe permutation. One addressed request may combine companionship, travel, stewardship, and useful work, but the session must not manufacture a hazard, restart, or dependency failure merely to touch a contract. The first material player-visible blocker becomes the active tranche; keep that whole outcome active through repair and physical rerun, then commit one meaningful checkpoint and freeze the corrected seam.
...
- [ ] Next broad tranche: run Gabriel's ordinary landing-area request to gather a sensible starter supply, make whatever basic tools are actually needed, preserve the lived-in site, and return. Keep the whole outcome active and let its first player-visible authority, stewardship, no-progress, truthfulness, or missing-domain blocker select the repair.
