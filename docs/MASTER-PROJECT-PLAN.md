...
The next objective is a trustworthy supervised-play baseline, not release certification. Gabriel can begin useful playtesting once the bot reliably accepts only intended orders, preserves the lived-in world, reuses selected infrastructure, performs one ordinary resource/processing/storage job, and remains Stop-safe and truthful when blocked.
...
3. **Resource responsiveness:** use the stocking campaign to expose the first real food/acquisition blocker. If collection repeats no-progress signatures, repair the existing binding/strategy seam instead of increasing timeouts or retries.
4. **Sustained truthful play:** after the supervised stocking session works, run a longer mixed companion session with Stop/restart only when gameplay requires it. Fix stale ownership, misleading narration, or replay only if physically observed.
