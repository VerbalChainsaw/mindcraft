...
3. **Resource responsiveness:** the first real collection-heavy play request exposed partial-tree acceptance, now repaired at the shared natural-tree primitive. A lumberjack completes a started bounded component or reports its exact blocker; quantity-bound prerequisites still stop at quantity. If later collection repeats no-progress signatures, repair the existing binding/strategy seam instead of increasing timeouts or retries.
