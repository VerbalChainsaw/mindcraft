...
- [ ] Active player-selected blocker: lumberjack harvesting discovers the connected natural tree but can treat partial inventory gain as completion after bounded branch failures, leaving branched trees cored out. Repair the shared started-tree completion contract across straight, branching, large-trunk, and irregular-root shapes; do not add species recipes or a second locomotion engine.
...
3. **Resource responsiveness:** the first real collection-heavy play request exposed partial-tree acceptance. Finish the shared natural-tree primitive so a lumberjack completes a started connected tree or reports its exact bounded blocker; if later collection repeats no-progress signatures, repair the existing binding/strategy seam instead of increasing timeouts or retries.
...
The immediate working session begins with IronSuiteProof still Operator Stop-held. Gabriel joins the existing world and plays normally. One addressed broad request may combine companionship and useful work, but the session must not manufacture a hazard, restart, or dependency failure merely to exercise a checklist. The first material player-visible blocker becomes the active tranche; keep that whole outcome active through repair and physical rerun, then commit one meaningful checkpoint and freeze the corrected seam.
...
- [ ] Next broad tranche: materially different supervised play. Let an ordinary useful request select the next player-visible authority, stewardship, no-progress, truthfulness, or missing-domain blocker; do not choose another pen, food quantity, delivery amount, or infrastructure permutation in advance.
