...
Status: diagnostics plus two bounded Scenario Lab adapters are implemented, and both fixed-fixture decisive replays passed. Required independent repetitions, non-interference measurement, three unavailable families, and the full A0/lab gates remain incomplete.
...
| At least 10 independent runs per family | Missing | Stone recovery and doorway/corridor follow each passed one direct and one deterministic-NL invocation. Both remain below the independent-run gate. |
...
| Direct/NL allocation | Partial live evidence | Stone recovery and doorway/corridor follow each passed one direct and one deterministic-NL invocation; three families and required repetitions remain unrun. |
...
Three Scenario Lab families still lack safe executor/evidence adapters and immutable fixtures. Both runnable families passed their fixed-fixture direct plus deterministic-NL replays but still lack the required independent repetitions and instrumentation off/on comparison. Those gates must close before A0 or the lab can be declared complete.
