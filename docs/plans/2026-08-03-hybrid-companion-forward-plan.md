...
Freeze the completed river-companion, shared-outpost-stocking, lit-animal-pen, landing-supply, and shared-worksite campaigns. Randomized two-player play has now selected a real collection-strategy blocker through the unchanged natural request: `Find a useful nearby cave, collect some iron and coal without damaging our work area, then return to me.`

The durable request, protected departure, cave search, coal acquisition, truthful missing-iron failure, usable-surface recovery, and player return are preserved below. Keep the same request active and add one genuinely different deterministic acquisition strategy at the existing capability/planner binding seam, using the bounded mining-corridor mechanics already owned by the project. Switch only after comparable cave/exposed-ore no-progress evidence. Do not add an iron-specific route, raise attempt ceilings, invent another relocation loop, or create a learning framework or second executor.

Do not reopen food quantities, furnace states, chest contents, starter-supply quantities/order, river routes, gaze, fence species, pen-light permutations, or corrected cave/surface movement without new live evidence. Do not turn playtesting into a certification matrix, telemetry project, claimed-world framework, storage controller, or second executor.
