...
The escorted-worksite campaign now passes physically from one natural request: accompany the player across a river, settle with that player at the explicitly requested existing furnace, smelt the carried raw iron there, resume the preserved Follow directive, return across the river, and obey Stop. The three blockers were repaired at shared seams: Follow now judges liquid escape by convergence on the dry player and delegates the selected shore route to native Pathfinder; Agenda has one bounded typed `follow_until` condition so standing Follow can yield to a later existing capability; and all planners/smelters now share one vanilla fuel-output primitive, including fractional sapling fuel. No route, recipe, workstation controller, or parallel movement engine was added.

Preserve and review this checkpoint, then choose the next unfamiliar player-valued outcome at product scale. Prefer a request that combines several already-proven capabilities with one still-unproven companion responsibility, especially world stewardship, explicit building/site permission, natural order attribution, sustained useful work, or shared-infrastructure selection. Run the broad request first and let its first physical blocker select the repair. Do not rerun river, furnace, sapling-fuel, Follow, farm, delivery, or container permutations without a later broad regression.
