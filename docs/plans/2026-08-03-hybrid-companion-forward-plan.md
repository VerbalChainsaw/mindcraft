...
Jordan accepted the escorted-worksite result in `J2C-20260806-1349-escorted-worksite-binding-correction` subject to one exact handoff correction. A successful `follow_until` now attaches the observed block name, X/Y/Z, dimension, and source Agenda entry to the immediately dependent smelt entry in the same atomic store write that settles Follow. Restart reloads that typed constraint, dispatch uses the existing exact `!smeltItem` form, and the existing smelter fails closed if the designated furnace changed instead of selecting a substitute. One short Paper proof introduced a decoy furnace only after furnace A was durably bound: B was 2.06 blocks from the bot versus A at 3.20, A visibly held the raw iron and fuel while B stayed empty and unlit, the smelt completed, and the fixture was restored. The full river route was not rerun.

Freeze shoreline, `follow_until`, furnace selection, fractional fuel, and return-Follow permutations. The Director's next live request, `build a shelter`, is the next broad product-scale campaign because it exercises an unproven responsibility: explicit building permission and world stewardship around a shared player site. Observe the whole existing builder outcome first. Descend only at its first material gameplay blocker; do not prebuild a structure registry, ownership framework, new builder, or another workstation campaign.
