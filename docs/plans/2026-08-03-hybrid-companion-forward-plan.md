...
Preserve the completed broad farm companion checkpoint and request the required full-companion review. Do not rerun Follow, husk, gaze, farm, container, or Agenda permutations. After review, advance to one product-scale scenario that combines the remaining major tranches—non-destructive travel, swimming where naturally required, and reuse of player-designated infrastructure—rather than returning to item quantities or isolated companion mechanics. Keep the declarative catalogue, GoalDirector, ActionManager, owned package mechanics, Stop authority, and no-parallel-engine rules fixed; descend narrowly only when that broad scenario exposes its first shared blocker.
