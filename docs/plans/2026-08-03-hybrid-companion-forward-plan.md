...
Preserve `2bca9c6` and obtain checkpoint review. The exact-workstation slice is complete and must not become a series of furnace-quantity variants. Resume the product-scale rule: choose one broad, useful, unfamiliar companion outcome, run it physically first, and repair only the first shared capability or ownership blocker it exposes. Keep the declarative catalogue, GoalDirector, ActionManager, package-first mechanics, Stop authority, and world-stewardship rules fixed; do not add item-specific routes, another executor, a workstation controller, or speculative framework work.
