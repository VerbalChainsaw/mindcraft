...
Preserve the Paper-verified clock checkpoint and request the required read-only Jordan review. Then run one generic Phase 3 companion campaign on unchanged capability code: accept a player-authorized delivery outcome, acquire or craft it through the catalogue, survive any legitimate interruption, return near the player using native Pathfinder, and verify the physical handoff. The selected item must use existing connected-registry planning; it may not receive an item-specific route. Repair only the first shared return, delivery, ownership, or cancellation blocker exposed by that same campaign.
