...
Preserve `fd1d9c4`. The exact broad acceptance request remains `Bring me 8 iron ingots and use the furnace here.` Its first run delivered eight and exposed the now-repaired existing-workstation binder; the unchanged repeat failed upstream after four expensive, distinct ore-route rejections. Make candidate infeasibility cheap at the existing collection/mining boundary, ensure one failed acquisition plus relocation causes a materially different feasible strategy, and rerun the same request through the existing workshop furnace and verified delivery. Do not raise attempt ceilings, add iron-specific search, create a structure registry, add a second executor, or reopen settled delivery/workstation architecture.
