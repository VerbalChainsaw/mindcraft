...
Preserve `e210d75`. The exact broad acceptance request remains `Bring me 8 iron ingots and use the furnace here.` Mining, tool replacement, surface recovery, fuel acquisition, eight-item smelting, return, and verified delivery now pass from zero raw material. Carry the explicit worksite-furnace constraint across the typed-goal boundary, bind the existing workshop furnace without a local fallback, and rerun the same request to Paper-verified delivery. Keep the binding generic for planner-visible workstation operations; do not add an iron-specific route, structure registry, second executor, or workstation controller.
