...
The remembered-animal follow-on exposed useful generic seams but was stopped before full acceptance. The preserved implementation adds canonical renewable-crop knowledge, mature-crop acquisition and replanting, moving-region entity reacquisition, safe surface/perimeter stance binding, and repeated no-progress method exclusion. Live Minecraft proved target reacquisition, surface/perimeter routing, mature-crop interaction, and one physical gate crossing; it did **not** prove complete multi-animal containment and breeding. Freeze that unfinished edge case as deferred capability work rather than spending another tranche certifying sheep permutations.

The active lane is now **high-frequency companion playability polish**. Use one normal mixed session to exercise already-delivered behavior: stay near the player without collision or oscillation, traverse ordinary terrain non-destructively, gather useful common resources completely and with the right tool, yield immediately to changed instructions or Stop, narrate only current truthful state, return to the player, and settle calmly. Repair the first shared primitive or ownership defect that harms this whole loop; do not add a noun-specific route, a new framework, or a showcase-only behavior. Preserve the package-first movement mandate, generic capability catalogue, one ActionManager executor, and broad-run acceptance rule.
...
- [~] Preserve the generic renewable-resource and remembered-animal prototype without claiming the livestock outcome complete. Canonical crop/breeding knowledge, mature-crop harvest/replant, moving-region reacquisition, safe surface/perimeter stance binding, and repeated no-progress method exclusion are reusable; full containment/breeding remains deferred until ordinary play makes it important.
- [ ] Polish one high-frequency mixed companion loop end to end: natural Follow/presence, ordinary non-destructive movement, complete useful resource gathering, responsive interruption, truthful current narration, requester return, and calm settled idle.
