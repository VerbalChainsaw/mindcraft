...
## RT-085 - Ordinary worksite setup bypasses durable construction and can lose the requested return
- Reproduction: ask the bot to set up a shared work area with named functions and return to the player. `Set up` and the multiword worksite noun escaped the durable construction route, while curly punctuation and missing return resolution could leave an unresolved Agenda fragment.
- Root cause: construction recognition covered a narrower verb/noun vocabulary than the generic design boundary, required-function prompting named outcomes without mapping them to supported DSL operations, and Agenda normalization did not discard punctuation-only fragments.
- Fix: route ordinary construction verbs and worksite nouns through the existing construction barrier, supply generic function-to-DSL guidance, normalize curly punctuation, discard empty punctuation fragments, and resolve return phrases to the typed player-return step.
- Evidence: the unchanged request queued exactly two steps, produced one correlated 33-cell blueprint with access/crafting/smelting/storage/light, completed with zero productive failures, passed independent Paper cell verification, and returned beside LandingWitness at health 20.

## RT-086 - Command-only idle survival invents optional shelter work after a terminal player goal
- Reproduction: complete the shared-worksite return at ordinary night while the healthy, fed bot uses command autonomy. SurvivalDirector can immediately start an `emergency_shelter` job despite having no player-authorized work.
- Root cause: the command-autonomy effective policy suppressed optional debris collection but still inherited the global `shelter: emergency` setting, so normal nighttime relevance was enough to authorize a world-changing job.
- Fix: at the existing command-only policy boundary, suppress optional shelter and useful-drop work while retaining critical food/health recovery, self-defense, and player-defense.
- Evidence: focused survival policy tests cover optional shelter suppression and critical eating. In Paper at night, the full-health bot completed a fresh return, remained idle for about 19 seconds with no new work order, then accepted Stop after world time was restored.

## RT-087 - Later mining legs can destroy the saved route home and fill inventory with unretirable excavation output
- Reproduction: run the natural cave/iron/coal/return request after exposed-cave search switches to the deterministic mining corridor. The first implementation persisted each cleared route cell but planned each new leg without protecting the accumulated route. A later leg removed the support block beneath an earlier stair, so return repeatedly targeted the same now-unsafe cell. A fresh repeat preserved and retraced the route but exhausted coal acquisition when route preflight required more working slots than cleanup could release and most carried debris was cobblestone.
- Root cause: exact mining geometry protected only the current leg, retained expeditions duplicated final return ownership between a stale coordinate and the following player-relative Agenda step, and inventory cleanup reused the world-clearance classification. Cobblestone must remain protected as possible player construction in the world, but verified mining output needs distinct carried-item provenance. The cleanup loop also allowed a 12-slot request while capping itself at four release actions.
- Fix: propagate the complete persisted return route through `reach_mining_depth` and `advance_mining_corridor`; reject any candidate excavation intersecting a saved support/feet/head cell; retrace those cells with native Pathfinder and digging disabled; make an already-unsafe saved cell non-retryable; let the durable player-relative Agenda step solely own the retained expedition's last mile; align the bounded release-action ceiling with the 12-slot contract; and allow only mining-capacity cleanup to retire redundant carried cobblestone/cobbled deepslate while preserving at least 16 blocks. Generic world clearing and ordinary inventory cleanup remain unchanged.
- Evidence: 55 focused planner, Agenda, work-order, placement, navigation, and runtime checks passed. In Paper, the bot gained six raw iron and four coal, retraced from y23 to y111 through more than 100 protected cells, returned beside LandingWitness, and remained at health 20. A second run returned safely and truthfully exposed the cobblestone-capacity boundary. The mining-only debris selector regression passes while the existing world-cobblestone non-clearable checks remain green. One final all-in-one ordinary collection replay remains pending; this checkpoint does not claim full repeatability.
