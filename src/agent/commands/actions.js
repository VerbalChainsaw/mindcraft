...
export const actionsList = [
    ...
    },
    {
        ...
        compactDescription: 'Persist one complete validated blueprint before gathering. DESIGN MUST BE THIS DSL, NEVER PROSE. Separate operations with ;. Operations: box X Y Z W H D [MATERIAL]; shell X Y Z W H D [MATERIAL]; room X Y Z W H D [MATERIAL]; slab X Y Z W D [MATERIAL]; ring X Y Z W D [MATERIAL]; line X1 Y1 Z1 X2 Y2 Z2 [MATERIAL]; block X Y Z MATERIAL; roof X Y Z W D flat|gable|pyramid [MATERIAL]; carve X Y Z W H D; put X Y Z door|glass|torch|chest|ladder|fence|gate|crafting|furnace|bed [FACING]. Fixtures MUST use put, never block. Coordinates are relative x east/y up/z south, each 0..31. SUPPORT RULE: in room starting at y=0, the floor is y=0; put ground fixtures at y=1 or higher above solid floor. Wall fixtures such as torches go in interior air adjacent to a same-height solid wall. A door occupies its anchor plus the block directly above; leave both clear. A bed occupies its anchor plus one block in its facing direction; keep both over clear supported interior floor. Use material "auto" and lock_material false unless the player named the structural material.',
        params: {
