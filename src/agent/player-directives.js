...
export function resolvePlayerDirective(playerName, message, context = {}) {
    ...
    if (
        ...
        return {
            ...
            releasesHold: true,
            deferToModel: true,
            modelInstruction: 'This is one player-authorized multi-block construction outcome. Compile the complete bounded blueprint before acquiring materials. Return one complete !buildStructure or !designStructure command in the first response; do not mention a command name in prose before the executable command. The !designStructure design argument must use its exact compact DSL contract; descriptive prose is invalid. For a habitable building, prefer room or explicitly provide slab + shell + roof because shell contains walls only. Fixtures must use put, never block. Fixture facing, when supplied, must be north, south, east, or west. Put ground fixtures above solid floor and wall fixtures such as torches in interior air adjacent to a same-height solid wall. A bed occupies its anchor plus one block in its facing direction; keep both cells over clear supported interior floor. Never replace the roof or required support with a fixture. Set !designStructure lock_material true only if the player explicitly named the structural material; otherwise omit it or pass false so Builder binds one feasible safe material for the entire required quantity. The persistent Builder will derive and acquire every blueprint material, place supported cells, and verify the finished world state. Do not issue search, inventory, gathering, crafting, or individual placement commands first.',
        };
    }
